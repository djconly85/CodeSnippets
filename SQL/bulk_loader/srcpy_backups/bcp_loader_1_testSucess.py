...
def create_table(conn_info, sql_file, sql_table_name, overwrite=True):
    ...
    if len([t for t in tables if t == sql_table_name]) > 0:
...
if __name__ == '__main__':
    ...
    #================TEST ZONE======================
    #----------------TEST PARAMS---------------------
    test_tbl_sql = r"C:\Users\dconly\GitRepos\CodeSnippets\SQL\bulk_loader\create_hh_table.sql"
    ...
    rcode = subprocess.call(bcp_cmd) #shell=True
    if rcode == 0:
        print(f"\nRows successfully loaded to table {test_tbl_name}")
