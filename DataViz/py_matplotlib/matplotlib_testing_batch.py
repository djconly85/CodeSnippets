...
class BarChart():
    ...
    def make_bar_chart(self, ax, subplt_df, subplt_title=None, yval_numformat='numeric', label_precision=2):
        ...
        x_label_vals = subplt_df[self.subplt_xticklab].unique()  # dfp['measure'].unique()
        ...
        axislabel_x = dfp[self.subplt_xticklab].name
        axislabel_y = dfp[self.subplt_col_yvals].name # dfp['value'].name
        ...
        if self.subplt_series_vals is not None:
            data_series = subplt_df[self.subplt_series_vals].unique()  # dfp['data_year'].unique()
        else:
    ...
    def make_bar_chart_subplots(self):
        '''
